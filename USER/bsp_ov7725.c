...
/************配置1*********横屏显示*****************************/
	...
	.QVGA_VGA = 0,	//QVGA模式  240(height)*320(width)
	.cam_sx = 0,
	.cam_sy = 0,	
...
/* 寄存器参数配置  格式： address, value */
Reg_Info Sensor_Config[] =
{
	{REG_CLKRC,     0x00}, /*clock config*/
	{REG_COM7,      0x46}, /*QVGA RGB565  240 * 340 */
	{REG_HSTART,    0x3f},
	{REG_HSIZE,     0x50},
	{REG_VSTRT,     0x03},
	{REG_VSIZE,     0x78},
...
{    //引脚配置成为输出模式， PC8-15: data0-7配置成为输入
...
ErrorStatus OV7725_Init(void)
{
	...
	if( 0 == SCCB_WriteByte ( 0x12, 0x80 ) ) /*复位0v7725 sensor */
	{   // 向0x12地址 写入 0x80命令，用于复位
	...
	OV7725_DEBUG("Sensor ID is 0x%x", Sensor_IDCode);	
	...
	if(Sensor_IDCode == OV7725_ID)
	{
		for( i = 0 ; i < OV7725_REG_NUM ; i++ )
		{
			...
			{                
                //OV7725_DEBUG("write reg faild", Sensor_Config[i].Address);
				OV7725_DEBUG("write reg faild");
                return ERROR;
			}
		}
	}
...
//读fifo的数据，并且将它显示到LCD屏幕上
//修改该函数，读fifo数据，然后将数据发送到串口上；
